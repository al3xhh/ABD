...
public class AbedemonDAOImpl implements AbedemonDAO {
    ...
    public List<Abedemon> getAbedemonsOf(String type) 
    {
		...
		try 
		{
			...
			while(rs.next())
			{
				String abedemon = rs.getItemAsString(new Properties());
				int indexNumAtaques = abedemon.indexOf("num-ataques="), indexNombre = abedemon.indexOf("nombre="), 
						indexId = abedemon.indexOf("id="), nAtaques = Integer.parseInt(abedemon.substring(indexNumAtaques + 13, indexNombre - 2));
				String nombre = abedemon.substring(indexNombre + 8, indexId - 2), id = abedemon.substring(indexId + 4, abedemon.length() - 3);
